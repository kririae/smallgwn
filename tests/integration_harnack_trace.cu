...
template <int Order> struct TraceResults {
...
TraceResults<Order> run_trace(
    ...
    std::vector<Real> const &dz, Real const epsilon = Real(1e-3), int const max_iter = 2048,
    ...
    s = gwn::gwn_compute_harnack_trace_batch_bvh_taylor<Order, Real, Index>(
        ...
        d_ny.span(), d_nz.span(), Real(0.5), epsilon, max_iter, t_max, Real(2)
    );
    if (!s.is_ok()) {
...
TEST_F(CudaFixture, integration_harnack_octahedron_sphere_rays) {
    ...
    for (std::size_t i = 0; i < n; ++i) {
        ...
        EXPECT_GT(hr, Real(0.2)) << "ray " << i << ": hit too close to origin (r=" << hr << ")";
        EXPECT_LT(hr, Real(1.0)) << "ray " << i << ": hit too far from origin (r=" << hr << ")";
    }
    ...
    for (std::size_t i = 0; i < n; ++i) {
        if (res1.t[i] < Real(0))
            continue;
        Real const nmag =
            std::sqrt(res1.nx[i] * res1.nx[i] + res1.ny[i] * res1.ny[i] + res1.nz[i] * res1.nz[i]);
    ...
    std::cout << "[harnack integration] Max t-diff Order-0 vs 1: " << max_t_diff_01
              << "  Order-1 vs 2: " << max_t_diff_12 << "  (compared " << compared << " rays)\n";
    ...
    for (std::size_t i = 0; i < n; ++i) {
        ...
        if (z_face > Real(0)) {
            ...
            EXPECT_GE(res.t[i], t_face - Real(2e-3))
                << "ray " << i
                << ": angle-mode hit should not lie meaningfully before face crossing";
...
TEST_F(CudaFixture, integration_harnack_cube_sphere_rays) {
    ...
    for (std::size_t i = 0; i < n; ++i) {
        ...
        Real const hz = oz[i] + res1.t[i] * dz[i];
        Real const hr = std::sqrt(hx * hx + hy * hy + hz * hz);
        EXPECT_GT(hr, Real(0.8)) << "ray " << i << ": hit too close to origin (r=" << hr << ")";
        EXPECT_LT(hr, Real(2.0)) << "ray " << i << ": hit too far from origin (r=" << hr << ")";
    }
