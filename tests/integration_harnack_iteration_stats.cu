...
__global__ void trace_collect_iters_kernel(
    ...
    int const max_iterations, Real const t_max, Real const accuracy_scale, IterStats *out_stats
) {
...
void measure_iteration_stats(
    ...
    std::vector<Real> const &dz, Real const epsilon, int const max_iterations, Real const t_max,
    Real const accuracy_scale, char const *label
) {
    ...
    trace_collect_iters_kernel<Order><<<grid, block>>>(
        ...
        d_oy.data(), d_oz.data(), d_dx.data(), d_dy.data(), d_dz.data(), n, Real(0.5), epsilon,
        max_iterations, Real(100), accuracy_scale, d_out.data()
    );
    ...
    auto print_stats = [](char const *tag, std::vector<int> &iters) {
        ...
        int p99 = iters[std::min(
            iters.size() - 1, static_cast<std::size_t>(static_cast<double>(iters.size()) * 0.99)
        )];
        std::cout << "  " << tag << ": n=" << iters.size() << " avg=" << avg << " min=" << min_v
                  << " median=" << median << " p90=" << p90 << " p99=" << p99 << " max=" << max_v
                  << "\n";
    };

    std::cout << "\n=== " << label << " ===\n";
    std::cout << "  total rays=" << n << " hits=" << hit_iters.size()
...
TEST_F(CudaFixture, iteration_stats_closed_octahedron) {
    ...
    measure_iteration_stats<1>(
        mesh, ox, oy, oz, dx, dy, dz, Real(1e-3), 2048, Real(100), Real(2),
