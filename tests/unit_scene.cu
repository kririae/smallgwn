...
TEST_F(CudaFixture, SimilarityTransformIdentity) {
    gwn::gwn_similarity_transform<Real> const default_transform{};
    auto const identity_transform = gwn::gwn_similarity_transform<Real>::identity();

    std::array<Real, 3> const point{{Real(1.5), Real(-2.0), Real(0.25)}};
    std::array<Real, 3> const direction{{Real(-3.0), Real(4.0), Real(2.5)}};
    std::array<Real, 3> const default_point = apply_point_host(default_transform, point);
    std::array<Real, 3> const default_direction =
        apply_direction_host(default_transform, direction);
    std::array<Real, 3> const identity_point = apply_point_host(identity_transform, point);
    std::array<Real, 3> const identity_direction =
        apply_direction_host(identity_transform, direction);

    EXPECT_NEAR(default_point[0], point[0], Real(1e-6));
    EXPECT_NEAR(default_point[1], point[1], Real(1e-6));
    EXPECT_NEAR(default_point[2], point[2], Real(1e-6));
    EXPECT_NEAR(default_direction[0], direction[0], Real(1e-6));
    EXPECT_NEAR(default_direction[1], direction[1], Real(1e-6));
    EXPECT_NEAR(default_direction[2], direction[2], Real(1e-6));
    EXPECT_NEAR(identity_point[0], point[0], Real(1e-6));
    EXPECT_NEAR(identity_point[1], point[1], Real(1e-6));
    EXPECT_NEAR(identity_point[2], point[2], Real(1e-6));
    EXPECT_NEAR(identity_direction[0], direction[0], Real(1e-6));
    EXPECT_NEAR(identity_direction[1], direction[1], Real(1e-6));
    EXPECT_NEAR(identity_direction[2], direction[2], Real(1e-6));

    gwn::gwn_aabb<Real> const local{Real(-1), Real(-2), Real(-3), Real(4), Real(5), Real(6)};
    gwn::gwn_aabb<Real> const default_world = default_transform.transform_aabb(local);
    gwn::gwn_aabb<Real> const identity_world = identity_transform.transform_aabb(local);
    EXPECT_NEAR(default_world.min_x, local.min_x, Real(1e-6));
    EXPECT_NEAR(default_world.min_y, local.min_y, Real(1e-6));
    EXPECT_NEAR(default_world.min_z, local.min_z, Real(1e-6));
    EXPECT_NEAR(default_world.max_x, local.max_x, Real(1e-6));
    EXPECT_NEAR(default_world.max_y, local.max_y, Real(1e-6));
    EXPECT_NEAR(default_world.max_z, local.max_z, Real(1e-6));
    EXPECT_NEAR(identity_world.min_x, local.min_x, Real(1e-6));
    EXPECT_NEAR(identity_world.min_y, local.min_y, Real(1e-6));
    EXPECT_NEAR(identity_world.min_z, local.min_z, Real(1e-6));
    EXPECT_NEAR(identity_world.max_x, local.max_x, Real(1e-6));
    EXPECT_NEAR(identity_world.max_y, local.max_y, Real(1e-6));
    EXPECT_NEAR(identity_world.max_z, local.max_z, Real(1e-6));
}
