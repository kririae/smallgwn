...
enum ImGuiFileBrowserFlags_ : std::uint32_t {
    ...
    ImGuiFileBrowserFlags_SkipItemsCausingError =
        1 << 10, // when entering a new directory, any error will interrupt the process, causing the
                 // file browser to fall back to the working directory. with this flag, if an error
                 // is caused by a specific item in the directory, that item will be skipped,
...
private:
    ...
    bool SetCurrentDirectoryInternal(
        std::filesystem::path const &dir, std::filesystem::path const &preferredFallback
    );
...
inline ImGui::FileBrowser::FileBrowser(
    ...
    : width_(700), height_(450), posX_(0), posY_(0), flags_(flags),
      defaultDirectory_(std::move(defaultDirectory)), shouldOpen_(false), shouldClose_(false),
      isOpened_(false), isOk_(false), isPosSet_(false), rangeSelectionStart_(0), editDir_(false),
...
inline ImGui::FileBrowser::FileBrowser(FileBrowser const &copyFrom) : FileBrowser() {
    *this = copyFrom;
}
...
inline void ImGui::FileBrowser::Display() {
    ...
    if (shouldOpen_ && (flags_ & ImGuiFileBrowserFlags_NoModal)) {
        ...
        if (isPosSet_) {
            SetNextWindowPos(
                ImVec2(static_cast<float>(posX_), static_cast<float>(posY_)), ImGuiCond_FirstUseEver
            );
        }
        SetNextWindowSize(
            ImVec2(static_cast<float>(width_), static_cast<float>(height_)), ImGuiCond_FirstUseEver
        );
    }
    ...
    if (editDir_) {
        ...
        if (enter) {
            ...
            if (is_directory(enteredDir)) {
                ...
                statusStr_ =
                    "[" + std::string(currDirBuffer_.data()) + "] is not a valid directory";
            }
        }
    } else {
    ...
    if (SmallButton("*")) {
        ...
        for (auto &name : selectedFilenames_) {
            auto const it = std::find_if(
                ...
                [&](FileRecord const &record) { return name == record.name; }
            );
            if (it != fileRecords_.end())
                newSelectedFilenames.insert(name);
        }

        if ((flags_ & ImGuiFileBrowserFlags_EnterNewFilename) && !inputNameBuffer_.empty() &&
            inputNameBuffer_[0]) {
...

    {
        BeginChild(
            ...
            (flags_ & ImGuiFileBrowserFlags_NoModal) ? ImGuiWindowFlags_AlwaysHorizontalScrollbar
                                                     : 0
        );
...
inline std::filesystem::path const &ImGui::FileBrowser::GetDirectory() const noexcept {
...
inline void ImGui::FileBrowser::ToolTip(std::string_view const &s) {
    if (!ImGui::IsItemHovered())
        return;
...
inline void ImGui::FileBrowser::UpdateFileRecords() {
    ...
    auto const getDirectoryIterator = [&]() -> std::filesystem::directory_iterator {
        try {
    ...
    for (auto &p : getDirectoryIterator()) {
        ...
        try {
            ...
            rcd.showName =
                (rcd.isDir ? "[D] " : "[F] ") + u8StrToStr(p.path().filename().u8string());
        } catch (...) {
            if (!(flags_ & ImGuiFileBrowserFlags_SkipItemsCausingError))
                throw;
            continue;
    ...
    if (fileRecords_.size() > 2) {
        ...
        for (auto &fileRecord : fileRecords_) {
            auto const name = u8StrToStr(fileRecord.name.u8string());
        ...
        std::sort(
            fileRecordRemapIndices.begin() + 1, fileRecordRemapIndices.end(),
        ...
        for (uint32_t const index : fileRecordRemapIndices)
...
inline void
ImGui::FileBrowser::AssignToArrayStyleString(std::vector<char> &arr, std::string_view content) {
    if (content.empty()) {
        if (!arr.empty())
            arr[0] = '\0';
        return;
...
#if defined(__cpp_lib_char8_t)
    // With C++20/23, it's impossible to efficiently convert a `char*` string to a `char8_t*` string
    // without violating the strict aliasing rule. Bad joke!
