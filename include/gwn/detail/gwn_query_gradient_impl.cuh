...
//
// For a triangle with vertices a, b, c and query point q, the gradient of the
// signed solid angle Ω(q) is:
//
//   ∇Ω = Σ_{edges (gi, gj)} [(gi − gj)·(ĝi − ĝj)] * (gi × gj) / |gi × gj|²
//
// where gi = pi − q for each vertex pi.  The sum runs over the 3 edges.
// ---------------------------------------------------------------------------
...
__device__ inline gwn_query_vec3<Real> gwn_winding_gradient_point_bvh_taylor_impl(
    ...
    GWN_ASSERT(!taylor_nodes.empty(), "winding gradient: taylor_nodes empty for internal root");
    stack[stack_size++] = bvh.root_index;
    while (stack_size > 0) {
        Index const node_index = stack[--stack_size];
        GWN_ASSERT(stack_size >= 0, "winding gradient: stack underflow");
        ...
        for (int child_slot = 0; child_slot < Width; ++child_slot) {
            ...
            if (!descend) {
                ...
                if constexpr (Order >= 1) {
                    // Order-1 gradient: ∇ω₁ = (1/r⁴)[(15S − 3T)r̂ − 3(v+u)]
                    //
                    // ω₁ = −Σ N_{αj} ∂_α ∂_j K  ⟹  ∇_m ω₁ = −Σ N_{αj} ∂_m ∂_α ∂_j K
                    //
                    // After contracting ∂³K with N_{αj}:
                    //   ∇_m ω₁ = (1/r⁴)[(15S − 3T)r̂_m − 3(v_m + u_m)]
                    ...
                    Real const S = qnx * qnx * Nij_xx + qny * qny * Nij_yy +
                                   qnz * qnz * Nij_zz + qnx * qny * Nxy_Nyx +
                                   qnx * qnz * Nzx_Nxz + qny * qnz * Nyz_Nzy;

                    // (v+u)_m = Σ_j (N_mj + N_jm) r̂_j
                    // Since we store N_xy + N_yx as child_nxy_nyx, the
                    // symmetrised matrix is [[2Nxx, Nxy+Nyx, Nzx+Nxz],
                    //                        [Nxy+Nyx, 2Nyy, Nyz+Nzy],
                    //                        [Nzx+Nxz, Nyz+Nzy, 2Nzz]]
                    Real const vu_x =
                ...
                if constexpr (Order >= 2) {
                    // Order-2 gradient: ∇_m ω₂ = (1/2) N_{ijk} ∂_m∂_i∂_j∂_k K(r)
                    //
                    // Factored form (verified against exact tensor contraction):
                    //   ∇_m ω₂ = (1/r⁵)[3/2 P_m − 15/2 (r̂_m L + R_m) + 105/2 r̂_m C]
                    //
                    // where P, L, C are the SAME quantities from ω₂, and R_m is a
                    // new quadratic form using the same 10 stored moment combinations.
                    Real const qlength_m5 = qlength_m2 * qlength_m2 * qlength_m1;
                    ...
                    // P_m: same linear coefficients used in ω₂.
                    ...
                    Real const C2 =
                        qnx2 * qnx * nijk_xxx + qny2 * qny * nijk_yyy +
                        qnz2 * qnz * nijk_zzz + qnx * qny * qnz * spn +
                        qnx2 * (qny * Axy + qnz * Axz) +
                        qny2 * (qnz * Ayz + qnx * Ayx) +
                        qnz2 * (qnx * Azx + qny * Azy);

                    // R_m: new quadratic form per gradient component.
                    Real const Rx =
                ...
                if (isfinite(grad_approx_x) && isfinite(grad_approx_y) &&
            ...
            // Leaf: brute-force per-triangle gradient.
