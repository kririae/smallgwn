...
/// \brief Compute additive Taylor moments from one canonical triangle record.
template <int Order, gwn_real_type Real>
__device__ inline bool gwn_compute_triangle_taylor_raw_moment(
    gwn_bvh_triangle<Real> const &triangle, gwn_device_taylor_moment<Order, Real> &raw_moment
) noexcept {
    Real const ax = triangle.v0_x;
    Real const ay = triangle.v0_y;
    Real const az = triangle.v0_z;
    Real const bx = ax + triangle.e1_x;
    Real const by = ay + triangle.e1_y;
    Real const bz = az + triangle.e1_z;
    Real const cx = ax + triangle.e2_x;
    Real const cy = ay + triangle.e2_y;
    Real const cz = az + triangle.e2_z;
    ...
    if constexpr (Order >= 2) {
        ...
        auto compute_integrals = [&](int const va, int const vb, int const vc, Real *out_ii,
                                     Real *out_ij, Real *out_ik, int const i) {
            ...
            *out_ii = area_scale_a * (Real(0.5) * Pai * Pai + Real(2.0 / 3.0) * Pai * oab_i +
                                      Real(0.25) * oab_i * oab_i) +
                      area_scale_c * (Real(0.5) * Pci * Pci + Real(2.0 / 3.0) * Pci * ocb_i +
                                      Real(0.25) * ocb_i * ocb_i);
            ...
            auto compute_cross = [&](int const jk, Real const diff, Real *out) {
                ...
                *out =
                    area_scale_a * (Real(0.5) * Pai * Paj + Real(1.0 / 3.0) * Pai * oabmidj +
                                    Real(1.0 / 3.0) * Paj * oab_i + Real(0.25) * oab_i * oabmidj) +
                    area_scale_c * (Real(0.5) * Pci * Pcj + Real(1.0 / 3.0) * Pci * ocbmidj +
                                    Real(1.0 / 3.0) * Pcj * ocb_i + Real(0.25) * ocb_i * ocbmidj);
            };
        ...
        if (dx > Real(0)) {
            compute_integrals(
                order_a[0][0], order_a[0][1], order_a[0][2], &integral_xx,
                (dx >= dy && dy > Real(0)) ? &integral_xy : nullptr,
                (dx >= dz && dz > Real(0)) ? &integral_zx : nullptr, 0
            );
        }
        if (dy > Real(0)) {
            compute_integrals(
                order_a[1][0], order_a[1][1], order_a[1][2], &integral_yy,
                (dy >= dz && dz > Real(0)) ? &integral_yz : nullptr,
                (dx < dy && dx > Real(0)) ? &integral_xy : nullptr, 1
            );
        }
        if (dz > Real(0)) {
            compute_integrals(
                order_a[2][0], order_a[2][1], order_a[2][2], &integral_zz,
                (dx < dz && dx > Real(0)) ? &integral_zx : nullptr,
                (dy < dz && dy > Real(0)) ? &integral_yz : nullptr, 2
            );
        }
...
/// \brief Transient parent and arrival state for canonical BVH bottom-up propagation.
template <gwn_index_type Index> struct gwn_bvh_refit_state {
    cuda::std::span<Index> parent{};
    cuda::std::span<std::uint32_t> parent_slot{};
    cuda::std::span<std::uint32_t> arity{};
    cuda::std::span<unsigned int> arrivals{};
};

/// \brief Validate packed child ranges and recover canonical BVH parent links.
template <int Width, gwn_real_type Real, gwn_index_type Index>
struct gwn_prepare_bvh_refit_functor {
    ...
    void __device__ operator()(std::size_t const node_index) const noexcept {
        ...
        if (node_index >= bvh.nodes.size() || node_index >= state.parent.size() ||
            node_index >= state.parent_slot.size() || node_index >= state.arity.size()) {
            signal_error();
            return;
        }
        ...
        for (int child_slot = 0; child_slot < Width; ++child_slot) {
            ...
            if (child.is_internal()) {
                if (child.offset() >= bvh.nodes.size()) {
                    signal_error();
                    continue;
                }
                auto const child_index = static_cast<std::size_t>(child.offset());
                // Query nodes omit parent links. Refit recovers them once in transient storage;
                // the builder's tree invariant gives each internal child exactly one writer.
                state.parent[child_index] = static_cast<Index>(node_index);
                state.parent_slot[child_index] = static_cast<std::uint32_t>(child_slot);
                continue;
            }

            std::uint64_t const begin = child.offset();
            std::uint64_t const count = child.primitive_count();
            if (begin > bvh.triangles.size() || count > bvh.triangles.size() - begin)
                signal_error();
        }

        state.arity[node_index] = arity;
        if (arity == 0)
            signal_error();
    }
};
...
struct gwn_bvh_moment_refit_traits {
    using payload_type = gwn_device_taylor_moment<Order, Real>;
    using output_node_type = gwn_bvh_moment_node<Width, Order, Real>;
    ...
    __device__ static bool make_leaf_payload(
        gwn_bvh_accessor<Width, Real, Index> const &bvh, gwn_bvh_child<Real> const &leaf,
        ...
        for (std::uint32_t primitive_offset = 0; primitive_offset < leaf.primitive_count();
            ...
            if (!gwn_compute_triangle_taylor_raw_moment<Order>(
                    bvh.triangles[static_cast<std::size_t>(sorted_index)], primitive_payload
                ))
                return false;
    ...
    __device__ static void write_valid(
        output_context const &context, gwn_aabb<Real> const &bounds, std::size_t const node_id,
        int const child_slot, payload_type const &payload
    ) noexcept {
        ...
        if constexpr (Order >= 2) {
            ...
            Real const x_xx = payload.raw_x_xx - Real(2) * Px * payload.nx_x + Px * Px * Nx;
            Real const x_yy = payload.raw_x_yy - Real(2) * Py * payload.nx_y + Py * Py * Nx;
            Real const x_zz = payload.raw_x_zz - Real(2) * Pz * payload.nx_z + Pz * Pz * Nx;
            Real const x_xy =
            ...
            Real const y_xx = payload.raw_y_xx - Real(2) * Px * payload.ny_x + Px * Px * Ny;
            Real const y_yy = payload.raw_y_yy - Real(2) * Py * payload.ny_y + Py * Py * Ny;
            Real const y_zz = payload.raw_y_zz - Real(2) * Pz * payload.ny_z + Pz * Pz * Ny;
            Real const y_xy =
            ...
            Real const z_xx = payload.raw_z_xx - Real(2) * Px * payload.nz_x + Px * Px * Nz;
            Real const z_yy = payload.raw_z_yy - Real(2) * Py * payload.nz_y + Py * Py * Nz;
            Real const z_zz = payload.raw_z_zz - Real(2) * Pz * payload.nz_z + Pz * Pz * Nz;
            Real const z_xy =
