...
template <class Real, class Index>
__global__ void gwn_compute_triangle_aabbs_and_morton_kernel(
    const gwn_geometry_accessor<Real, Index> geometry,
    const Real scene_min_x,
    const Real scene_min_y,
    const Real scene_min_z,
    const Real scene_inv_x,
    const Real scene_inv_y,
    const Real scene_inv_z,
    gwn_aabb<Real>* primitive_aabbs,
    std::uint32_t* morton_codes,
    Index* primitive_indices) {
  const std::size_t triangle_id = static_cast<std::size_t>(blockIdx.x) *
                                      static_cast<std::size_t>(blockDim.x) +
                                  static_cast<std::size_t>(threadIdx.x);
  if (triangle_id >= geometry.triangle_count()) {
    return;
  }
...
template <class Real, class Index>
__global__ void gwn_gather_sorted_aabbs_kernel(
    const gwn_aabb<Real>* unsorted_aabbs,
    const Index* sorted_primitive_indices,
    gwn_aabb<Real>* sorted_aabbs,
    const std::size_t primitive_count) {
  const std::size_t primitive_id = static_cast<std::size_t>(blockIdx.x) *
                                       static_cast<std::size_t>(blockDim.x) +
                                   static_cast<std::size_t>(threadIdx.x);
...
template <class Real, class Index>
__global__ void gwn_build_leaf_entries_kernel(
    const gwn_aabb<Real>* sorted_aabbs,
    const std::size_t primitive_count,
    const std::size_t leaf_primitive_capacity,
    gwn_build_entry<Real, Index>* leaf_entries,
    const std::size_t leaf_count) {
  const std::size_t leaf_id = static_cast<std::size_t>(blockIdx.x) *
                                  static_cast<std::size_t>(blockDim.x) +
                              static_cast<std::size_t>(threadIdx.x);
...
template <class Real, class Index>
__global__ void gwn_build_bvh4_parent_level_kernel(
    const gwn_build_entry<Real, Index>* child_entries,
    const std::size_t child_count,
    gwn_build_entry<Real, Index>* parent_entries,
    gwn_bvh4_node_soa<Real, Index>* parent_nodes,
    const std::size_t parent_count) {
  const std::size_t parent_id = static_cast<std::size_t>(blockIdx.x) *
                                    static_cast<std::size_t>(blockDim.x) +
                                static_cast<std::size_t>(threadIdx.x);
  if (parent_id >= parent_count) {
    return;
  }
...
template <class Real, class Index>
__global__ void gwn_patch_child_indices_kernel(
    gwn_bvh4_node_soa<Real, Index>* nodes,
    const std::size_t node_count,
    const Index child_level_offset) {
  const std::size_t node_id = static_cast<std::size_t>(blockIdx.x) *
                                  static_cast<std::size_t>(blockDim.x) +
                              static_cast<std::size_t>(threadIdx.x);
...
gwn_status gwn_build_bvh4_lbvh(gwn_geometry_accessor<Real, Index>& accessor,
  ...
  const auto block_count_for = [](const std::size_t n) {
    return static_cast<int>((n + static_cast<std::size_t>(k_block_size) - 1) /
  ...
  detail::gwn_compute_triangle_aabbs_and_morton_kernel<Real, Index>
      <<<block_count_for(primitive_count), k_block_size, 0, stream>>>(
          accessor, scene_min_x, scene_min_y, scene_min_z, scene_inv_x,
          scene_inv_y, scene_inv_z,
          thrust::raw_pointer_cast(primitive_aabbs.data()),
          thrust::raw_pointer_cast(morton_codes.data()),
          thrust::raw_pointer_cast(sorted_primitive_indices.data()));
  status = gwn_cuda_to_status(cudaGetLastError());
  if (!status.is_ok()) {
    return status;
  }
  ...
  detail::gwn_gather_sorted_aabbs_kernel<Real, Index>
      <<<block_count_for(primitive_count), k_block_size, 0, stream>>>(
          thrust::raw_pointer_cast(primitive_aabbs.data()),
          thrust::raw_pointer_cast(sorted_primitive_indices.data()),
  ...
  status = detail::gwn_copy_device_to_span(
      accessor.bvh.primitive_indices,
      thrust::raw_pointer_cast(sorted_primitive_indices.data()),
  ...
  thrust::device_vector<detail::gwn_build_entry<Real, Index>> current_entries(
      leaf_count);
  detail::gwn_build_leaf_entries_kernel<Real, Index>
      <<<block_count_for(leaf_count), k_block_size, 0, stream>>>(
          thrust::raw_pointer_cast(sorted_aabbs.data()), primitive_count,
          k_leaf_primitive_capacity,
  ...
  while (current_count > 1) {
    ...
    detail::gwn_build_bvh4_parent_level_kernel<Real, Index>
        <<<block_count_for(parent_count), k_block_size, 0, stream>>>(
            thrust::raw_pointer_cast(current_entries.data()), current_count,
            thrust::raw_pointer_cast(parent_entries.data()),
  ...
  for (std::size_t level = 0; level + 1 < level_count; ++level) {
    detail::gwn_patch_child_indices_kernel<Real, Index>
        <<<block_count_for(level_node_counts[level]), k_block_size, 0,
           stream>>>(final_nodes + level_offsets[level],
